CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Create_Merge_Rename_24`(IN GT_DB VARCHAR(100),IN TABLE_NAME VARCHAR(100))
BEGIN
	...
	WHILE v_k < MAX_HR DO
	BEGIN
		SET qry_tbl_name=CONCAT(TABLE_NAME,'_',RIGHT(CONCAT(RIGHT(CONCAT('0',v_k),2)),4));
