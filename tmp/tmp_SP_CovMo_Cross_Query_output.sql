CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_CovMo_Cross_Query_output`(IN PU_ID VARCHAR(1000),IN START_DATE DATE,IN END_DATE DATE,IN sql_str LONGTEXT,IN V_WORK_ID INT(11),TECH_MASK TINYINT(4),IN PLOYGON_ID VARCHAR(100),IN IMSI_GID INT(11),IN CELL_GID INT(11),PRE_GROUP BIT,IN OUTPUT TINYINT(2),IN PATH VARCHAR(100),IN FILENAME VARCHAR(50))
a_label:
BEGIN
	/*This SP id for FET CSFB_indicator output */
	...
	DECLARE LIMT_RAW_COUNT SMALLINT(6) DEFAULT '-1';
	...
	ELSE
		...
		IF pre_group THEN
			SET @col_str_from_dis=CONCAT(@col_str_from_table,@col_str_from_where,@col_str_from_group,' ORDER BY NULL');		
			...
			WHILE @j_i <= @v_T_Max DO
				BEGIN
					...
					IF @SESSION_CNT>0 THEN 
						...
						ELSE 
							CALL gt_schedule.sp_job_add_task(CONCAT('CALL gt_gw_main.SP_CovMo_Cross_Query_Parallel(''',GT_DB,''',''',SESSION_NAME,''',''',v_source_table,''',''',CONCAT(CCQ_TABLE,'_',REPLACE(SESSION_NAME,'_0000_0000',''),'_',gt_strtok(gt_covmo_csv_get(PU_ALL,@v_i),2,'|')),''',''',CONCAT(' SELECT ',@result_col,CONCAT(',''''',gt_strtok(gt_covmo_csv_get(PU_ALL,@v_i),1,'|'),''''''),' ',REPLACE(@col_str_from_dis,"'","''")),''',''',v_schema,''',''',WORKER_ID,''',''',v_select,''',''',gt_strtok(gt_covmo_csv_get(PU_ALL,@v_i),2,'|'),''',''',PLOYGON_ID,''',',IMSI_GID,',',CELL_GID,',''',DS_AP_IP,''',''',DS_AP_PORT,''',''',@AP_USER,''',''',@AP_PSWD,''',',LIMT_RAW_COUNT,');'),@V_Multi_PU); 
						END IF;
		...
		IF @JOB_STATUS='FINISHED' THEN 
			...
			IF V_WORK_ID > 100 THEN
				IF PRE_GROUP THEN 
					IF OUTPUT=1 THEN 
						SET @SqlCmd=REPLACE(CONCAT('SELECT ''',REPLACE(mrg_str,',',''','''),''' UNION SELECT ',mrg_str,' ',REPLACE(@col_str_from_mrg,', pus AS pus',''),' INTO OUTFILE ''',PATH,FILENAME,''' FIELDS TERMINATED BY ''\\t'' LINES TERMINATED BY ''\\n'' ;'),v_source_table,CONCAT(GT_DB,'.',CCQ_TABLE));
					ELSE 
						SET @SqlCmd=REPLACE(CONCAT('SELECT SQL_CALC_FOUND_ROWS ',mrg_str,' ',@col_str_from_mrg,';'),v_source_table,CONCAT(GT_DB,'.',CCQ_TABLE));
					END IF;
				ELSE 
					IF OUTPUT=1 THEN 
						SET @SqlCmd=REPLACE(CONCAT('SELECT ''',REPLACE(upd_sql_str,',',''','''),''' UNION SELECT ',upd_sql_str,' ',REPLACE(@col_str_from_mrg,', pus AS pus',''),' INTO OUTFILE ''',PATH,FILENAME,''' FIELDS TERMINATED BY ''\\t'' LINES TERMINATED BY ''\\n'';'),v_source_table,CONCAT(GT_DB,'.',CCQ_TABLE));					
					ELSE 
