CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_CovMo_Cross_Query_20170825`(IN PU_ID VARCHAR(1000),IN START_DATE DATE,IN END_DATE DATE,IN sql_str LONGTEXT,IN V_WORK_ID INT(11),TECH_MASK TINYINT(4),IN PLOYGON_ID VARCHAR(100),IN IMSI_GID INT(11),IN CELL_GID INT(11),IN PRE_GROUP BIT,IN LIMIT_COUNT SMALLINT(6),IN LIMT_RAW_COUNT SMALLINT(6))
a_label:
BEGIN
	...
	ELSE
		...
		IF LOCATE('LEFT JOIN ',@col_str_from_other)>0 THEN 
			SET @col_str_join=REPLACE(REPLACE(@col_str,'|',','),'''','''''');
			SET @col_join_exist=1;
			...
			SET @col_str_join_col1=CONCAT(TRIM(SUBSTRING_INDEX(@col_str_from_table,' AS',-1)),'.');
			SET @col_str=REPLACE(@col_str,@col_str_join_col1,''); 			
		
 			SET @col_join_cnt=gt_covmo_csv_count(@col_str_from,' ON ')-1;
 			
			SET r=1;
			SET @col_str_join_col='';
			WHILE r <= @col_join_cnt DO
			BEGIN	
  				SET @col_str_join_col=CONCAT(TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(`SPLIT_STR`(@col_str_from_join,' LEFT JOIN ',r+1),' ON',1),' AS',-1)),'.');
				SET @col_str=REPLACE(@col_str,@col_str_join_col,'');
				SET r = r + 1;
			END;
			END WHILE;
		ELSE 
			SET @col_join_exist=0;
			SET @col_str_from_join='';
			SET @col_str_from_table=@col_str_from_other;
		END IF;
	
		IF pre_group THEN
			SET @col_str_from_dis=CONCAT(@col_str_from_table,@col_str_from_where,@col_str_from_group,IF( LIMIT_COUNT>0,@col_str_from_order,' ORDER BY NULL'));		
			SET @col_str_from_mrg=CONCAT(@col_str_pus,@col_str_from_table,@col_str_from_group,@col_str_from_having,@col_str_from_order,IF(LIMIT_COUNT>0,CONCAT(' LIMIT ',LIMIT_COUNT),@col_str_from_limit));
		ELSE 
			SET @col_str_from_dis=CONCAT(@col_str_from_table,@col_str_from_join,@col_str_from_where);
			SET @col_str_from_mrg=CONCAT(@col_str_pus,@col_str_from_table,@col_str_from_group,@col_str_from_having,@col_str_from_order,IF(LIMIT_COUNT>0,CONCAT(' LIMIT ',LIMIT_COUNT),@col_str_from_limit));
			...
			BEGIN
				...
				BEGIN			   
					...
					SET v_select =CONCAT(v_select,@col_str_col,',');
 					SET @col_str_order=@col_str_col;
				END;
				ELSEIF gt_strtok(@col_str, i, '|') REGEXP '\\(POW\\(.*\\)\\/COUNT\\(.*\\)\\)'>0 AND LOCATE('_POWER_',gt_strtok(@col_str, i, '|'))=0 THEN
				BEGIN			
					SET @col_str_source=TRIM(REPLACE(REPLACE(SUBSTRING_INDEX(gt_strtok(@col_str, i, '|'),' AS',1),'\r',''),'\n',''));
				...
				ELSEIF gt_strtok(@col_str, i, '|') REGEXP 'MIN\\(.*\\)'>0 THEN	
				BEGIN		
					SET @col_str_source=TRIM(REPLACE(REPLACE(SUBSTRING_INDEX(gt_strtok(@col_str, i, '|'),' AS',1),'\r',''),'\n',''));
					IF LEFT(@col_str_source,1)='(' AND RIGHT(@col_str_source,1)=')' THEN
					  SET @col_str_source=SUBSTR(@col_str_source,2,LENGTH(@col_str_source)-2);
					...
					SET @col_str_org=gt_strtok(@col_str, i, '|');
					SET @col_str_dis=CONCAT(@col_str_source,' AS ',@col_str_col);
					SET @col_str_mrg=CONCAT('MIN(',@col_str_col,') AS ',@col_str_col);
					IF @col_str_convert>0  THEN			
						SET v_schema =CONCAT(v_schema,@col_str_col,' FLOAT,');
 						SET @col_str_order=@col_str_col;
					ELSE
						SET v_schema =CONCAT(v_schema,@col_str_col,' VARCHAR(100),');
						SET @col_str_order=CONCAT('MIN(',@col_str_col,')');
					END IF;
					SET v_select =CONCAT(v_select,@col_str_col,',');
				END;
				ELSEIF gt_strtok(@col_str, i, '|') REGEXP 'GROUP_CONCAT\\(.*\\)'>0 THEN	
				BEGIN		
					SET @col_str_source=TRIM(REPLACE(REPLACE(SUBSTRING_INDEX(gt_strtok(@col_str, i, '|'),' AS',1),'\r',''),'\n',''));
					IF LEFT(@col_str_source,1)='(' AND RIGHT(@col_str_source,1)=')' THEN
					  SET @col_str_source=SUBSTR(@col_str_source,2,LENGTH(@col_str_source)-2);
					...
					IF LOCATE('SEPARATOR',@col_str_source)>0 THEN
					BEGIN
						SET @col_str_col_SEP=TRIM(SUBSTRING_INDEX(@col_str_source, 'SEPARATOR',-1));
						SET @col_str_col_SEP=TRIM(gt_strtok(@col_str_col_SEP, 1,'\''));
						SET @col_str_mrg=REPLACE(CONCAT('GROUP_CONCAT(',@col_str_col,' SEPARATOR ''',@col_str_col_SEP,''')  AS ',@col_str_col),',','|');
						SET v_schema =CONCAT(v_schema,@col_str_col,' VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_swedish_ci,');
						SET @col_str_order=CONCAT('GROUP_CONCAT(',@col_str_col,' SEPARATOR ''',@col_str_col_SEP,''')');
					END;
					ELSE
					BEGIN
						SET @col_str_mrg=CONCAT('GROUP_CONCAT(',@col_str_col,') AS ',@col_str_col);
						SET v_schema =CONCAT(v_schema,@col_str_col,' VARCHAR(100),');
						SET @col_str_order=CONCAT('GROUP_CONCAT(',@col_str_col,')');
					END;
					END IF;
				END;	
			...
			WHILE @j_i <= @v_T_Max DO
				BEGIN
					...
					IF @SESSION_CNT>0 THEN 
						...
						SET @SqlCmd=CONCAT('CREATE TABLE ',GT_DB,'.',CCQ_TABLE,'_',REPLACE(SESSION_NAME,'_0000_0000',''),'_',gt_strtok(gt_covmo_csv_get(PU_ALL,@v_i),2,'|'),v_schema,' ENGINE=MYISAM DEFAULT CHARSET=latin1 COLLATE  latin1_german2_ci;');
						PREPARE Stmt FROM @SqlCmd;
						EXECUTE Stmt;
						DEALLOCATE PREPARE Stmt;
						IF PRE_GROUP THEN 
							CALL gt_schedule.sp_job_add_task_upd(CONCAT('CALL gt_gw_main.SP_CovMo_Cross_Query_Parallel_20170825(''',GT_DB,''',''',SESSION_NAME,''',''',v_source_table,''',''',CONCAT(CCQ_TABLE,'_',REPLACE(SESSION_NAME,'_0000_0000',''),'_',gt_strtok(SPLIT_STR(PU_ALL,',',@v_i),2,'|')),''',''',CONCAT(' SELECT ',REPLACE(dis_str,"'","''"),CONCAT(',''''',gt_strtok(SPLIT_STR(PU_ALL,',',@v_i),1,'|'),''''''),' ',REPLACE(@col_str_from_dis,"'","''")),''',''',v_schema,''',''',WORKER_ID,''',''',v_select,''',''',gt_strtok(SPLIT_STR(PU_ALL,',',@v_i),2,'|'),''',''',PLOYGON_ID,''',',IMSI_GID,',',CELL_GID,',''',DS_AP_IP,''',''',DS_AP_PORT,''',''',@AP_USER,''',''',@AP_PSWD,''',',LIMT_RAW_COUNT,');'),@V_Multi_PU); 
						ELSE 
							CALL gt_schedule.sp_job_add_task_upd(CONCAT('CALL gt_gw_main.SP_CovMo_Cross_Query_Parallel_20170825(''',GT_DB,''',''',SESSION_NAME,''',''',v_source_table,''',''',CONCAT(CCQ_TABLE,'_',REPLACE(SESSION_NAME,'_0000_0000',''),'_',gt_strtok(SPLIT_STR(PU_ALL,',',@v_i),2,'|')),''',''',CONCAT(' SELECT ',@result_col,CONCAT(',''''',gt_strtok(SPLIT_STR(PU_ALL,',',@v_i),1,'|'),''''''),' ',REPLACE(@col_str_from_dis,"'","''")),''',''',v_schema,''',''',WORKER_ID,''',''',v_select,''',''',gt_strtok(SPLIT_STR(PU_ALL,',',@v_i),2,'|'),''',''',PLOYGON_ID,''',',IMSI_GID,',',CELL_GID,',''',DS_AP_IP,''',''',DS_AP_PORT,''',''',@AP_USER,''',''',@AP_PSWD,''',',LIMT_RAW_COUNT,');'),@V_Multi_PU);  
						END IF;
		...
		IF @JOB_STATUS='FINISHED' THEN 
			...
			IF V_WORK_ID > 100 THEN
				IF PRE_GROUP THEN 
					IF LIMIT_COUNT>0 THEN 
						SET @SqlCmd=REPLACE(CONCAT('SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  ',mrg_str,' ',@col_str_from_mrg,') AS fact ',@col_str_from_limit,';'),v_source_table,CONCAT(GT_DB,'.',CCQ_TABLE));
					ELSE 
			...
			ELSE 
				IF PRE_GROUP THEN 
					IF LIMIT_COUNT>0 THEN 
						SET @SqlCmd=REPLACE(CONCAT('SELECT * FROM (SELECT ',mrg_str,' ',@col_str_from_mrg,') AS fact ',@col_str_from_limit,';'),v_source_table,CONCAT(GT_DB,'.',CCQ_TABLE));
					ELSE
