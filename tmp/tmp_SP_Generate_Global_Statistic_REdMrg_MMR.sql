CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Generate_Global_Statistic_REdMrg_MMR`(IN TECH_MASK TINYINT(2),IN v_DATA_DATE_d DATETIME,IN KIND CHAR(2),IN IMSI_CELL TINYINT(2))
BEGIN	
	...
	DECLARE group_db_name VARCHAR(100) DEFAULT '';
	...
	BEGIN
		SET group_db_name=CONCAT('gt_global_statistic_g',gt_strtok(@REG_GROUP, @v_reg_m, '|'));
		SELECT `DAY_OF_WEEK`(v_DATA_DATE_d) INTO @DAY_OF_WEEK;  
		SET @FIRST_DAY_WK=gt_strtok(@DAY_OF_WEEK, 1, '|');
		SET @END_DAY_WK=gt_strtok(@DAY_OF_WEEK, 2, '|');
		
		IF KIND='MN' THEN 
			SET @END_DAY_MN=LAST_DAY(v_DATA_DATE_d);
			SET @SqlCmd=CONCAT('SELECT DATEDIFF(''',@END_DAY_MN,''',''',v_DATA_DATE_d,''') INTO @DiffDate ;');
			SET @DATE_MN=DATE_FORMAT(v_DATA_DATE_d,'%Y%m');
		ELSE 
			SET @SqlCmd=CONCAT('SELECT DATEDIFF(''',@END_DAY_WK,''',''',v_DATA_DATE_d,''') INTO @DiffDate ;');
			SET @DATE_WK=CONCAT(DATE_FORMAT(@FIRST_DAY_WK,'%Y%m%d'),'_',DATE_FORMAT(@END_DAY_WK,'%Y%m%d'));
		END IF;
		PREPARE Stmt FROM @SqlCmd;
		EXECUTE Stmt;
		DEALLOCATE PREPARE Stmt;
