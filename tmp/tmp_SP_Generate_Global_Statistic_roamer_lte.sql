CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Generate_Global_Statistic_roamer_lte`(IN DATA_DATE DATE,IN DATA_HOUR TINYINT(2),IN PU_ID MEDIUMINT(9),IN TECH_MASK TINYINT(2), IN GT_DB VARCHAR(100),IN TileResolution VARCHAR(10))
BEGIN
	...
	SET STR_START_END_LTE=CONCAT('POS_FIRST_TILE,MCC,MNC,
 					POS_FIRST_LOC, 					
					POS_FIRST_S_EARFCN ,
	...
	SET STR_START_SUM_LTE=CONCAT('POS_FIRST_TILE AS TILE_ID,MCC,MNC,
	...
	SET STR_END_SUM_LTE=CONCAT('POS_LAST_TILE AS TILE_ID,MCC,MNC,
					...
					SUM(UL_THROUPUT) AS `UL_THROUPUT_SUM`,
					COUNT(UL_THROUPUT) AS `UL_THROUPUT_CNT`,
					SUM(DL_THROUPUT) AS `DL_THROUPUT_SUM`,
					COUNT(DL_THROUPUT) AS `DL_THROUPUT_CNT`,
	...
	INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Generate_Global_Statistic_romer_lte',CONCAT(GT_DB,' CREATE TEMPORARY TABLE tmp_materialization_',WORKER_ID), NOW());	
	...
	INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Generate_Global_Statistic_romer_lte',CONCAT(GT_DB,' INSERT INTO tbl_',GT_DB,'_',WORKER_ID), NOW());	
	...
	BEGIN	
		...
		BEGIN		
			...
			ELSE
				SET @UNION=' UNION ALL '; 
			END IF;	
			SET STR_SEL_START_LTE=CONCAT(STR_SEL_START_LTE,'SELECT ',STR_START_END_LTE,' FROM ',GT_DB,'.table_roamer_call_lte',CONCAT('_',LPAD(@v_i,2,'0'),LPAD(@v_j,2,'0')),' WHERE DATA_HOUR=', @v_i,@UNION); 
			SET STR_SEL_END_LTE=CONCAT(STR_SEL_END_LTE,'SELECT ',STR_START_END_LTE,' FROM ',GT_DB,'.table_roamer_call_lte',CONCAT('_',LPAD(@v_i,2,'0'),LPAD(@v_j,2,'0')),' WHERE DATA_HOUR=', @v_i,@UNION); 
			SET @v_j=@v_j+15;
		END;
		END WHILE;
	...
	SET @SqlCmd=CONCAT('INSERT INTO ',GT_DB,'.tmp_tile_start_',WORKER_ID,' 
				(TILE_ID,MCC,MNC,',@TILE_ID_LVL2,',',@TILE_ID_LVL1,',EARFCN,EUTRABAND,DATA_DATE,DATA_HOUR
					,INIT_CALL_CNT,SIGNAL_CNT,DATA_CNT,UNSPECIFIED_CNT,BLOCK_CNT,CALL_SETUP_TIME_SUM,CALL_SETUP_TIME_CNT,
				...
				GROUP BY POS_FIRST_TILE,MCC,MNC,POS_FIRST_S_EARFCN,POS_FIRST_S_EUTRABAND
				ORDER BY NULL;
				');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	...
	SET @SqlCmd=CONCAT('INSERT INTO ',GT_DB,'.tmp_tile_end_',WORKER_ID,' 
				...
				,PS_UL_VOLUME_SUM,PS_DL_VOLUME_SUM,PS_UL_SPEED_MAX,PS_DL_SPEED_MAX,DROP_SIGNAL_CNT,DROP_DATA_CNT,`DROP_SMS_CNT`,`DROP_VOLTE_CNT`,
				`DROP_UNSPECIFIED_CNT`,`END_NON_BLOCK_CALL_CNT`,
				`UL_THROUPUT_SUM`,`UL_THROUPUT_CNT`,`DL_THROUPUT_SUM`,`DL_THROUPUT_CNT`,`END_NON_BLOCK_SIGNAL_CNT`,`END_NON_BLOCK_DATA_CNT`
				)
				SELECT ',STR_END_SUM_LTE,' 
				FROM (',STR_SEL_END_LTE,') AA
				GROUP BY POS_LAST_TILE,MCC,MNC,POS_LAST_S_EARFCN,POS_LAST_S_EUTRABAND
				ORDER BY NULL;
				');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	...
	SET @SqlCmd=CONCAT('INSERT INTO ',GT_DB,'.`tmp_materialization_',WORKER_ID,'` 
				( TILE_ID,MCC,MNC,EARFCN,EUTRABAND,DATA_DATE,DATA_HOUR)
			SELECT TILE_ID,MCC,MNC,EARFCN,EUTRABAND,DATA_DATE,DATA_HOUR
			FROM 
			(
				SELECT TILE_ID,MCC,MNC,EARFCN,EUTRABAND,DATA_DATE,DATA_HOUR FROM ',GT_DB,'.tmp_tile_start_',WORKER_ID,'
				UNION 
				SELECT TILE_ID,MCC,MNC,EARFCN,EUTRABAND,DATA_DATE,DATA_HOUR FROM ',GT_DB,'.tmp_tile_end_',WORKER_ID,'
			) AA;');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	...
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Generate_Global_Statistic_romer_lte',CONCAT(GT_DB,' END'), NOW());
