CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Generate_IMSI_PU`(IN GT_DB_QRT VARCHAR(100),TECH_MASK TINYINT(2))
BEGIN
	DECLARE PU_ID INT;
	...
	INSERT INTO gt_gw_main.SP_LOG VALUES(GT_DB_QRT,'SP_Generate_IMSI_PU',CONCAT(GT_DB_QRT,' START'), START_TIME);
	...
	INSERT INTO gt_gw_main.SP_LOG VALUES(GT_DB_QRT,'SP_Generate_IMSI_PU',CONCAT('CREATE tmp table ',GT_DB,'.tmp_imsi_',PU_ID,'_',WORKER_ID,' cost:',TIMESTAMPDIFF(SECOND,STEP_START_TIME,SYSDATE()),' sec.'), NOW());
	SET STEP_START_TIME := SYSDATE();
	...
	SET @SqlCmd=CONCAT('INSERT INTO ',GT_DB,'.table_imsi_diff_',PU_ID,'_',DATE_SH_EH,'_tmp 
				...
				WHERE NOT EXISTS 
				(SELECT NULL FROM ',GT_DB,'.table_imsi B WHERE B.IMSI=A.IMSI);');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	
	INSERT INTO gt_gw_main.SP_LOG VALUES(GT_DB_QRT,'SP_Generate_IMSI_PU',CONCAT('CREATE table ',GT_DB,'.table_imsi_diff_',PU_ID,'_',DATE_SH_EH,'_tmp cost:',TIMESTAMPDIFF(SECOND,STEP_START_TIME,SYSDATE()),' sec.'), NOW());
	SET STEP_START_TIME := SYSDATE();
	
	SET @SqlCmd=CONCAT('INSERT INTO ',GT_DB,'.table_imsi
				SELECT IMSI FROM ',GT_DB,'.table_imsi_diff_',PU_ID,'_',DATE_SH_EH,'_tmp 
				;');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	
	INSERT INTO gt_gw_main.SP_LOG VALUES(GT_DB_QRT,'SP_Generate_IMSI_PU',CONCAT('INSERT table ',GT_DB,'.table_imsi cost:',TIMESTAMPDIFF(SECOND,STEP_START_TIME,SYSDATE()),' sec.'), NOW());
	SET STEP_START_TIME := SYSDATE();
	...
	INSERT INTO gt_gw_main.SP_LOG VALUES(GT_DB_QRT,'SP_Generate_IMSI_PU',CONCAT('rename table ',GT_DB,'.table_imsi_diff_',PU_ID,'_',DATE_SH_EH,'_tmp cost:',TIMESTAMPDIFF(SECOND,STEP_START_TIME,SYSDATE()),' sec.'), NOW());
	SET STEP_START_TIME := SYSDATE();
	...
	INSERT INTO gt_gw_main.SP_LOG VALUES(GT_DB_QRT,'SP_Generate_IMSI_PU',CONCAT(GT_DB_QRT,' Done: ',TIMESTAMPDIFF(SECOND,START_TIME,SYSDATE()),' seconds.'), NOW());
