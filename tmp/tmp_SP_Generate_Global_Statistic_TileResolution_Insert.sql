CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Generate_Global_Statistic_TileResolution_Insert`(IN TECH_MASK TINYINT(2),IN TBL_NAME VARCHAR(100),IN TMP_TBL_NAME VARCHAR(100),IN TIMETYPE CHAR(2),IN PENDING_FLAG TINYINT(2),IN TileResolution VARCHAR(10))
BEGIN
	...
	DECLARE TILE_UMTS_ROAMER_COLUMN_STR VARCHAR(1500) DEFAULT 
		'`INIT_CALL_CNT`,
		  `END_CALL_CNT`,
		  `VOICE_CNT`,
		  `VIDEO_CNT`,
	...
	DECLARE TILE_UMTS_ROAMER_COLUMN_IFNULL_STR VARCHAR(5000) DEFAULT 
		'IFNULL(INIT_CALL_CNT,0) AS INIT_CALL_CNT,
		IFNULL(END_CALL_CNT,0) AS END_CALL_CNT,
		IFNULL(VOICE_CNT,0) AS VOICE_CNT,
		IFNULL(VIDEO_CNT,0) AS VIDEO_CNT,
	...
	DECLARE TILE_UMTS_ROAMER_COLUMN_SUM_STR VARCHAR(5000) DEFAULT 
		'SUM(IFNULL(INIT_CALL_CNT,0)) AS INIT_CALL_CNT,
		SUM(IFNULL(END_CALL_CNT,0)) AS END_CALL_CNT,
		SUM(IFNULL(VOICE_CNT,0)) AS VOICE_CNT,
		SUM(IFNULL(VIDEO_CNT,0)) AS VIDEO_CNT,
	...
	DECLARE TILE_GSM_ROAMER_COLUMN_STR VARCHAR(1500) DEFAULT 
		'INIT_CALL_CNT, 
		  END_CALL_CNT,
		  VOICE_CNT,
		  SIGNAL_CNT,
	...
	DECLARE TILE_GSM_ROAMER_COLUMN_IFNULL_STR VARCHAR(2000) DEFAULT 
		'IFNULL(INIT_CALL_CNT,0) AS INIT_CALL_CNT,           
		IFNULL(END_CALL_CNT,0) AS END_CALL_CNT,          
		IFNULL(VOICE_CNT,0) AS VOICE_CNT,           
		IFNULL(SIGNAL_CNT,0) AS SIGNAL_CNT,          
	...
	DECLARE TILE_GSM_ROAMER_COLUMN_SUM_STR VARCHAR(2000) DEFAULT 
		'SUM(IFNULL(INIT_CALL_CNT,0)) AS INIT_CALL_CNT,           
		SUM(IFNULL(END_CALL_CNT,0)) AS END_CALL_CNT,          
		SUM(IFNULL(VOICE_CNT,0)) AS VOICE_CNT,
		SUM(IFNULL(SIGNAL_CNT,0)) AS SIGNAL_CNT,
	...
	BEGIN	
		...
		INSERT INTO gt_gw_main.SP_LOG VALUES(@global_db,'SP_Generate_Global_Statistic_TileResolution_Insert_HR',CONCAT('Done: ',TIMESTAMPDIFF(SECOND,START_TIME,SYSDATE()),' seconds.'), NOW());
		ELSE
		SET @SqlCmd=CONCAT('INSERT INTO ',ZOOM_TBL_NAME,' 
				...
				,CASE WHEN ((PENDING_FLAG=1 AND TIMETYPE='HR') OR TIMETYPE IN ('DY','WK','MN')) THEN 
					CONCAT(' ON DUPLICATE KEY UPDATE ',REPLACE(@TILE_COLUMN_UPD_STR,'RPT_TABLE_NAME',ZOOM_TBL_NAME))
								ELSE '' END
				,';');
