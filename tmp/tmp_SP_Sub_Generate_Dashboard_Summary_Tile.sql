...
BEGIN
	...
	SET @SQLCMD=CONCAT('CREATE TABLE ',GT_DB,RUN,'.tmp_table_dashboard_summary_tile ENGINE=MYISAM 
				SELECT * FROM ',GT_DB,RUN,'.table_dashboard_summary_tile
				WHERE 1<>1;');
	PREPARE STMT FROM @SQLCMD;
	EXECUTE STMT;
	DEALLOCATE PREPARE STMT;
	SET @SQLCMD =CONCAT('INSERT INTO ',GT_DB,RUN,'.tmp_table_dashboard_summary_tile
				 (DATA_DATE,DATA_HOUR,TILE_ID,RNC_ID,NORMAL_CNT,DROP_CNT,BLOCK_CNT,CAll_CNT,RSCP,ECN0)
				 SELECT DATA_DATE,DATA_HOUR,TILE_ID, RNC_ID,
					 ...
					 SUM(BEST_RSCP_1*CALL_CNT)/SUM(CALL_CNT) RSCP,
					 SUM(BEST_ECN0_1*CALL_CNT)/SUM(CALL_CNT) ECN0
				 FROM ',GT_DB,RUN,'.table_tile_end 
