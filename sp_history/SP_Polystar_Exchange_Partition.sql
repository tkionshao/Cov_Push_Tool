...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Polystar_Exchange_Partition`(IN TECH_MASK TINYINT(2),IN GT_DB VARCHAR(50),IN gt_polystar_db VARCHAR (20),IN RPT_TYPE VARCHAR(20),IMSI_GID TINYINT (2))
BEGIN
  ...
  --	RPT_TYPE=1(DPI_CALL_QR),2(RPT_HR),3(RPT_DY),4(RPT_WK)
	INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition','START TO EXCHANGE PARTITION',NOW());	
	
	IF TECH_MASK=4 AND RPT_TYPE = '1' THEN 
	
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('START TO EXCHANGE PARTITION TABLE_POS_POLYSTAR_LTE_',DATA_QRT,''),NOW());	
		...
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('FINISH TO EXCHANGE PARTITION TABLE_POS_POLYSTAR_LTE_',DATA_QRT,''),NOW());	
	
	END IF;
	IF TECH_MASK=4 AND RPT_TYPE = '2' THEN 
	
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('START TO EXCHANGE PARTITION RPT_POLYSTAR_LTE_',DATA_HOUR,''),NOW());	
	
		SET @SqlCmd=CONCAT('SELECT GROUP_CONCAT(DISTINCT `TABLE_NAME` ORDER BY `TABLE_NAME` SEPARATOR ''|'') INTO @TABLE_GROUP_HR 
					FROM information_schema.`TABLES` 
					WHERE TABLE_SCHEMA=''',gt_polystar_db,''' AND TABLE_NAME LIKE ''%lte_hr'' AND TABLE_NAME NOT LIKE ''%xdr%'';');
		PREPARE Stmt FROM @SqlCmd;
		EXECUTE Stmt;
		DEALLOCATE PREPARE Stmt;
		...
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('FINISH TO EXCHANGE PARTITION RPT_POLYSTAR_LTE_',DATA_HOUR,''),NOW());	
	END IF;
	
	IF TECH_MASK=4 AND RPT_TYPE = '3' THEN 
	
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('START TO EXCHANGE PARTITION RPT_POLYSTAR_LTE_',DATA_DATE,''),NOW());	
	
		SET @SqlCmd=CONCAT('SELECT GROUP_CONCAT(DISTINCT `TABLE_NAME` ORDER BY `TABLE_NAME` SEPARATOR ''|'') INTO @TABLE_GROUP_DY 
					FROM information_schema.`TABLES` 
					WHERE TABLE_SCHEMA=''',gt_polystar_db,''' AND TABLE_NAME LIKE ''%lte_dy'' AND TABLE_NAME NOT LIKE ''%xdr%'';');
		PREPARE Stmt FROM @SqlCmd;
		EXECUTE Stmt;
		DEALLOCATE PREPARE Stmt;
		...
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('FINISH TO EXCHANGE PARTITION RPT_POLYSTAR_LTE_',DATA_DATE,''),NOW());		
	END IF;
	
	IF TECH_MASK=2 AND RPT_TYPE = '1' THEN 
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('START TO EXCHANGE PARTITION TABLE_POS_POLYSTAR_UMTS_',DATA_QRT,''),NOW());	
		...
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('FINISH TO EXCHANGE PARTITION TABLE_POS_POLYSTAR_UMTS_',DATA_QRT,''),NOW());	
	END IF;
	
	IF TECH_MASK=2 AND RPT_TYPE = '2' THEN 
	
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('START TO EXCHANGE PARTITION RPT_POLYSTAR_UMTS_',DATA_HOUR,''),NOW());	
	
		SET @SqlCmd=CONCAT('SELECT GROUP_CONCAT(DISTINCT `TABLE_NAME` ORDER BY `TABLE_NAME` SEPARATOR ''|'') INTO @TABLE_GROUP_HR 
					FROM information_schema.`TABLES` 
					WHERE TABLE_SCHEMA=''',gt_polystar_db,''' AND TABLE_NAME LIKE ''%umts_hr'' AND TABLE_NAME NOT LIKE ''%xdr%'';');
		PREPARE Stmt FROM @SqlCmd;
		EXECUTE Stmt;
		DEALLOCATE PREPARE Stmt;
		...
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('FINISH TO EXCHANGE PARTITION RPT_POLYSTAR_UMTS_',DATA_HOUR,''),NOW());	
	END IF;
	
	IF TECH_MASK=2 AND RPT_TYPE = '3' THEN 
	
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('START TO EXCHANGE PARTITION RPT_POLYSTAR_UMTS_',DATA_DATE,''),NOW());	
	
		SET @SqlCmd=CONCAT('SELECT GROUP_CONCAT(DISTINCT `TABLE_NAME` ORDER BY `TABLE_NAME` SEPARATOR ''|'') INTO @TABLE_GROUP_DY 
					FROM information_schema.`TABLES` 
					WHERE TABLE_SCHEMA=''',gt_polystar_db,''' AND TABLE_NAME LIKE ''%umts_dy'' AND TABLE_NAME NOT LIKE ''%xdr%'';');
		PREPARE Stmt FROM @SqlCmd;
		EXECUTE Stmt;
		DEALLOCATE PREPARE Stmt;
		...
		INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Polystar_Exchange_Partition',CONCAT('FINISH TO EXCHANGE PARTITION RPT_POLYSTAR_UMTS_',DATA_DATE,''),NOW());		
	END IF;
	
	INSERT INTO gt_gw_main.SP_LOG VALUES(GT_DB,'SP_Polystar_Exchange_Partition',CONCAT('Done: ',TIMESTAMPDIFF(SECOND,START_TIME,SYSDATE()),' seconds.'), NOW());
END$$
DELIMITER ;
