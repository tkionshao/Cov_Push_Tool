...
BEGIN	
	...
	SET @SqlCmd=CONCAT('UPDATE ',GT_COVMO,'.TMP_NEW_TAC 
				SET strtok1=SPLIT_STR(UPPER(manufacturer),'' '',1),
				strtok2=SPLIT_STR(UPPER(manufacturer),'' '',2);');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	...
	INSERT INTO gt_gw_main.SP_LOG VALUES(GT_COVMO,'SP_Update_TAC','ADD NEW manufacturer', START_TIME);
 	...
 	SET @SqlCmd=CONCAT('INSERT INTO ',GT_COVMO,'.TMP_dim_handset_id(manufacturer,full_name)
				...
				AND SPLIT_STR(UPPER(manufacturer),'' '',1) NOT IN (SELECT (SPLIT_STR(UPPER(full_name),'' '',1)) FROM ',GT_COVMO,'.dim_handset_id)
				AND SPLIT_STR(UPPER(manufacturer),'' '',1) NOT IN (SELECT IFNULL((SPLIT_STR(UPPER(full_name),'' '',2)),''NULL'') FROM ',GT_COVMO,'.dim_handset_id);				
				');				
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	...
	SET @SqlCmd=CONCAT('INSERT INTO ',GT_COVMO,'.TMP_dim_handset_id(manufacturer,full_name)
				...
				AND manufacturer NOT IN (SELECT full_name FROM ',GT_COVMO,'.dim_handset_id)
				AND SPLIT_STR(UPPER(manufacturer),'' '',1) NOT IN (SELECT (SPLIT_STR(UPPER(full_name),'' '',1))FROM ',GT_COVMO,'.dim_handset_id)
				AND SPLIT_STR(UPPER(manufacturer),'' '',2) NOT IN (SELECT IFNULL((SPLIT_STR(UPPER(full_name),'' '',1)),''NULL'') FROM ',GT_COVMO,'.dim_handset_id)
				AND SPLIT_STR(UPPER(manufacturer),'' '',1) NOT IN (SELECT (SPLIT_STR(UPPER(full_name),'' '',2))FROM ',GT_COVMO,'.dim_handset_id)
				AND SPLIT_STR(UPPER(manufacturer),'' '',2) NOT IN (SELECT IFNULL((SPLIT_STR(UPPER(full_name),'' '',2)),''NULL'') FROM ',GT_COVMO,'.dim_handset_id);				
				');				
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
...
-- 				WHERE SPLIT_STR(UPPER(manufacturer),'' '',1) IN (SELECT UPPER(full_name) FROM ',GT_COVMO,'.TMP_dim_handset_id)
-- 				AND SPLIT_STR(UPPER(manufacturer),'' '',2) IN (SELECT SPLIT_STR(UPPER(full_name),'' '',2) FROM ',GT_COVMO,'.TMP_dim_handset_id) 
-- 				OR SPLIT_STR(UPPER(manufacturer),'' '',2) IN (SELECT UPPER(full_name) FROM ',GT_COVMO,'.TMP_dim_handset_id)
-- 				AND SPLIT_STR(UPPER(manufacturer),'' '',2) IN (SELECT SPLIT_STR(UPPER(full_name),'' '',2) FROM ',GT_COVMO,'.TMP_dim_handset_id);
-- 			   ');
	SET @SqlCmd=CONCAT('INSERT INTO ',GT_COVMO,'.TMP_dim_handset_id_1(make_id,manufacturer,full_name)
				SELECT make_id,manufacturer,full_name FROM GT_COVMO.TMP_dim_handset_id GROUP BY manufacturer HAVING COUNT(*)>1
				');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	...
	SET @SqlCmd=CONCAT('CREATE TEMPORARY TABLE ',GT_COVMO,'.TMP_dim_handset_id_1(
						`make_id` SMALLINT,
						`manufacturer` VARCHAR(50),
						`strtok1` VARCHAR(50),
						`strtok2` VARCHAR(50),
						KEY key_strtok1 (`strtok1`),
						KEY key_strtok2 (`strtok2`)
						) ENGINE=MYISAM DEFAULT CHARSET=LATIN1 DELAY_KEY_WRITE=1;');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;

	SET @SqlCmd=CONCAT('INSERT INTO ',GT_COVMO,'.TMP_dim_handset_id_1(make_id,manufacturer,strtok1,strtok2)
				SELECT make_id,manufacturer,
				IFNULL((SPLIT_STR(UPPER(manufacturer),'' '',1)),''NULL'') AS strtok1,IFNULL((SPLIT_STR(UPPER(manufacturer),'' '',2)),''NULL'') AS strtok2
