...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Generate_Global_Statistic_roamer_umts`(IN DATA_DATE DATE,IN DATA_HOUR TINYINT(2),IN PU_ID MEDIUMINT(9),IN TECH_MASK TINYINT(2), IN GT_DB VARCHAR(100),IN TileResolution VARCHAR(10))
BEGIN
	DECLARE WORKER_ID VARCHAR(10) DEFAULT CONNECTION_ID();
	INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Generate_Global_Statistic_roamer_umts',CONCAT(GT_DB,' CREATE TEMPORARY TABLE tmp_materialization_',WORKER_ID), NOW());	
	...
	INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Generate_Global_Statistic_roamer_umts',CONCAT(GT_DB,' INSERT INTO tbl_',GT_DB,'_',WORKER_ID), NOW());	
	...
	SET @SqlCmd=CONCAT('CREATE TEMPORARY TABLE ',GT_DB,'.tmp_tile_start_',WORKER_ID,' ENGINE=MYISAM AS
				...
				FROM ',GT_DB,'.table_roamer_call
				WHERE DATA_HOUR =',DATA_HOUR,' AND POS_FIRST_RNC=',PU_ID,' 
	...
	SET @SqlCmd=CONCAT('CREATE TEMPORARY TABLE ',GT_DB,'.tmp_tile_end_',WORKER_ID,' ENGINE=MYISAM AS
				...
				FROM ',GT_DB,'.table_roamer_call
				WHERE DATA_HOUR =',DATA_HOUR,' AND POS_LAST_RNC=',PU_ID,' 
	...
	SET @SqlCmd=CONCAT('CREATE INDEX `ix_tile_fq` ON ',GT_DB,'.tmp_tile_start_',WORKER_ID,' (TILE_ID,MCC,MNC,FREQUENCY,UARFCN);');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	
	SET @SqlCmd=CONCAT('CREATE INDEX `ix_tile_fq` ON ',GT_DB,'.tmp_tile_end_',WORKER_ID,' (TILE_ID,MCC,MNC,FREQUENCY,UARFCN);');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;	
	
	SET @SqlCmd=CONCAT('INSERT INTO ',GT_DB,'.`tmp_materialization_',WORKER_ID,'` 
				( TILE_ID,MCC,MNC,FREQUENCY,UARFCN,DATA_DATE,DATA_HOUR)
			SELECT TILE_ID,MCC,MNC,FREQUENCY,UARFCN,DATA_DATE,DATA_HOUR
			FROM 
			(
				SELECT TILE_ID,MCC,MNC,FREQUENCY,UARFCN,DATA_DATE,DATA_HOUR FROM ',GT_DB,'.tmp_tile_start_',WORKER_ID,'
				UNION 
				SELECT TILE_ID,MCC,MNC,FREQUENCY,UARFCN,DATA_DATE,DATA_HOUR FROM ',GT_DB,'.tmp_tile_end_',WORKER_ID,'
			) AA;');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	...
	SET @SqlCmd=CONCAT('UPDATE ',GT_DB,'.`tmp_materialization_',WORKER_ID,'` a,',GT_DB,'.`tmp_tile_start_',WORKER_ID,'` B
			...
			WHERE A.TILE_ID=B.TILE_ID AND 
					A.MCC=B.MCC AND
					A.MNC=B.MNC AND
					A.FREQUENCY=B.FREQUENCY AND A.UARFCN=B.UARFCN;');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	...
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	INSERT INTO gt_gw_main.sp_log VALUES('gt_gw_main','SP_Generate_Global_Statistic_roamer_umts',CONCAT(GT_DB,' END'), NOW());
