...
BEGIN
	...
	IF PLOYGON_ID>0 OR IMSI_GID>0 OR CELL_GID>0 THEN 
		...
		SET @SqlCmd=CONCAT('SELECT spider_bg_direct_sql(CONCAT(''CALL gt_gw_main.SP_CovMo_Cross_Query_Parallel_Remote_20170825(''''',REPLACE(REPLACE(sql_str,"'","''''"),v_source_table,CONCAT(session_db,'.',v_source_table)),''''',''''',@polygon_str,''''',''''',@imsi_group_id,''''',''''',CELL_GID,''''',''''',gt_strtok(session_db,2,'_'),''''',''''',PLOYGON_ID,''''',''''',LIMT_RAW_COUNT,''''');'') 
