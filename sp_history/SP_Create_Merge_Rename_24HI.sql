...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Create_Merge_Rename_24HI`(IN GT_DB VARCHAR(100),IN TBL_NAME VARCHAR(100), IN BOTTON_STR VARCHAR(100), IN SRC_TBL VARCHAR(100))
BEGIN
	...
	SET @SqlCmd=CONCAT('CREATE TABLE ',GT_DB,'.',TBL_NAME,' LIKE ',GT_DB,'.',SRC_TBL,';');
	...
	BEGIN	
		...
		BEGIN
			...
			SET qry_tbl_name=CONCAT(NEW_TBL_NAME,'_',RIGHT(CONCAT(RIGHT(CONCAT('0',v_k),2)),2),BOTTON_STR);
			
			SET @SqlCmd=CONCAT('DROP TABLE IF EXISTS ',GT_DB,'.',qry_tbl_name,';');
			PREPARE Stmt FROM @SqlCmd;
			EXECUTE Stmt;
			DEALLOCATE PREPARE Stmt;
			SET @SqlCmd=CONCAT('CREATE TABLE ',GT_DB,'.',qry_tbl_name,' LIKE ',GT_DB,'.',SRC_TBL,';');
			PREPARE Stmt FROM @SqlCmd;
			EXECUTE Stmt;
			DEALLOCATE PREPARE Stmt;
