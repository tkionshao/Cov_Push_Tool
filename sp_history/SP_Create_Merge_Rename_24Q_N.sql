...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Create_Merge_Rename_24Q_N`(IN GT_DB VARCHAR(100),IN TBL_NAME VARCHAR(100),IN TABLE_NUM TINYINT(2))
BEGIN
	...
	BEGIN	
		...
		BEGIN
			...
			BEGIN
				...
				SET qry_tbl_name=CONCAT(TBL_NAME,'_',RIGHT(CONCAT(RIGHT(CONCAT('0',v_k),2),RIGHT(CONCAT('0',v_i),2)),4),'_',v_j);
