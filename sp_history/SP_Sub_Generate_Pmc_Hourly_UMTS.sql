...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Sub_Generate_Pmc_Hourly_UMTS`(IN GT_DB VARCHAR(100))
BEGIN
	...
	INSERT INTO gt_gw_main.sp_log VALUES(O_GT_DB,'SP_Sub_Generate_Pmc_Hourly_UMTS','Start ', NOW());
	SET @SqlCmd=CONCAT('TRUNCATE TABLE ',DY_GT_DB,'.table_tile_pmc_c_',START_HH,' ;');
	...
	SET @SqlCmd=CONCAT('INSERT INTO ',DY_GT_DB,'.`tmp_table_call_',WORKER_ID,'`  
									...
									FROM ',DY_GT_DB,'.table_call_',START_HH,' 
									WHERE POS_FIRST_CELL IS NOT NULL
									GROUP BY DATA_DATE,DATA_HOUR,POS_FIRST_CLUSTER,POS_FIRST_FREQUENCY,POS_FIRST_UARFCN,POS_FIRST_CELL_INDOOR,POS_FIRST_SITE,POS_FIRST_RNC,POS_FIRST_CELL;');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;

	INSERT INTO gt_gw_main.sp_log VALUES(O_GT_DB,'SP_Sub_Generate_Pmc_Hourly_UMTS',CONCAT('Insert into table table_tile_pmc_c_',START_HH), NOW());
		
	SET @SqlCmd=CONCAT('INSERT INTO ',DY_GT_DB,'.`table_tile_pmc_c_',START_HH,'`
						SELECT 
							DATA_DATE,
							DATA_HOUR,
							CLUSTER_ID,
						...
						(
							...
							FROM `',DY_GT_DB,'`.table_tile_fp_c_def_',START_HH,'
							UNION ALL
							SELECT DATA_DATE,
								DATA_HOUR,
								CLUSTER_ID,
								...
								NULL AS IRAT_HO_PS_ATT,
								NULL AS IRAT_HO_PS_FAIL
							FROM `',DY_GT_DB,'`.table_tile_start_c_def_',START_HH,'
							UNION ALL
							SELECT DATA_DATE,
								DATA_HOUR,
								CLUSTER_ID,
								...
								NULL AS IRAT_HO_PS_ATT,
								NULL AS IRAT_HO_PS_FAIL
							FROM `',DY_GT_DB,'`.table_tile_end_c_def_',START_HH,'
							UNION ALL
							SELECT DATA_DATE,
								DATA_HOUR,
								CLUSTER_ID,
						...
						GROUP BY DATA_DATE,DATA_HOUR,CLUSTER_ID,FREQUENCY,UARFCN,CELL_INDOOR,SITE_ID,RNC_ID,CELL_ID;');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;	
	...
	INSERT INTO gt_gw_main.SP_LOG VALUES(O_GT_DB,'SP_Sub_Generate_Pmc_Hourly_UMTS',CONCAT('Done: ',TIMESTAMPDIFF(SECOND,START_TIME,SYSDATE()),' seconds.'), NOW());	
END$$
DELIMITER ;
