...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Generate_PU_LTE_FET_CEM`(IN GT_DB VARCHAR(100),IN GT_COVMO VARCHAR(100))
BEGIN
	...
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;	
	SET @SqlCmd=CONCAT('UPDATE ',GT_DB,'.`nt_cell_current_lte` A, gt_gw_main.fet_pu_cem B
				SET A.PU_ID = B.PU_ID
			WHERE A.CLUSTER_NAME_REGION = B.CLUSTER_NAME_REGION;');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
