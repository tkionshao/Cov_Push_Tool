...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Generate_PU_LTE_Viva`(IN GT_DB VARCHAR(100),IN GT_COVMO VARCHAR(100))
BEGIN
	...
	SET @SqlCmd=CONCAT('INSERT INTO ',GT_DB,'.`pu_enodeb_mapping_',WORKER_ID,'` 
				(ENODEB_ID,ENODEB_NAME,CLUSTER,AREA_NAME,VENDOR)
				SELECT DISTINCT
				  ENODEB_ID AS ENODEB_ID,
				  ENODEB_NAME AS ENODEB_NAME,
				  CLUSTER_NAME_REGION AS CLUSTER,
				  CLUSTER_NAME_SUB_REGION AS AREA_NAME,
	...
	SET @SqlCmd=CONCAT('CREATE TEMPORARY TABLE ',GT_DB,'.`tmp_dim_sub_region_mapping_',WORKER_ID,'`
		SELECT  B.PU_ID,B.AREA_NAME,B.VENDOR FROM ',GT_DB,'.`dim_sub_region_mapping_',WORKER_ID,'` A RIGHT JOIN (
			SELECT DISTINCT AREA_NAME,PU_ID,VENDOR FROM gt_gw_main.pu_enodeb_mapping WHERE AREA_NAME <> ''''
		) B 
		ON A.CLUSTER_NAME_SUB_REGION = B.area_name
