...
-- 	SELECT GROUP_CONCAT(DISTINCT (ABS(`GROUP_ID`))) INTO @imsig_value FROM `gt_covmo`.`dim_imsi_group` WHERE GROUP_ID < 0 AND GROUP_ID >= -10000;
	SET @SqlCmd=CONCAT('SELECT GROUP_CONCAT(DISTINCT (ABS(`GROUP_ID`))) INTO @imsig_value FROM ',CURRENT_NT_DB,'.`dim_imsi_group` WHERE GROUP_ID < 0 AND GROUP_ID >= -10000;');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	...
	IF FLAG='DY' THEN 
		...
		IF (VENDOR_ID & 1)>0 THEN
			CALL gt_gw_main.SP_Create_Merge_Rename_24Q(GT_DB,'table_pm_ericsson_umts');
			CALL gt_gw_main.SP_Create_Merge_Rename_24H(GT_DB,'table_pm_ericsson_umts_aggr');
			CALL gt_gw_main.SP_Create_Merge_Rename_24Q(GT_DB,'table_pm_pdf_ericsson_umts');
			CALL gt_gw_main.SP_Create_Merge_Rename_24H(GT_DB,'table_pm_pdf_ericsson_umts_aggr');
		END IF;
	END IF;	
