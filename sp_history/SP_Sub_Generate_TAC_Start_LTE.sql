...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Sub_Generate_TAC_Start_LTE`(IN GT_DB VARCHAR(100))
BEGIN
	DECLARE RNC_ID INT;
...
...sample';
	INSERT INTO gt_gw_main.sp_log VALUES(O_GT_DB,'SP_Sub_Generate_TAC_Start_LTE','Start', NOW());
	
	SET SUM_STR_COL = 'POS_FIRST_RSRP|POS_FIRST_RSRP_CNT|POS_FIRST_RSRQ|POS_FIRST_RSRQ_CNT|RRC_REQUEST_TYPE_1|RRC_REQUEST_TYPE_2|RRC_REQUEST_TYPE_3|RRC_REQUEST_TYPE_4|RRC_REQUEST_TYPE_5|RRC_REQUEST_TYPE_6|RRC_REQUEST_TYPE_7|RRC_REQUEST_TYPE_8|SERVING_CNT|POS_FIRST_S_RSRP|POS_FIRST_S_RSRP_CNT|POS_FIRST_S_RSRQ|POS_FIRST_S_RSRQ_CNT|MEASURE_CNT|IMSI_CNT|RRC_CONNECTION_REJECT|CA_INDICATOR_EARFCN_0|CA_INDICATOR_EARFCN_1|CA_INDICATOR_EARFCN_2|CA_INDICATOR_EARFCN_3|CA_INDICATOR_EARFCN_4|CA_INDICATOR_EARFCN_5|CA_INDICATOR_EARFCN_6|CA_INDICATOR_EARFCN_7|CA_INDICATOR_EARFCN_8|CA_POS_FIRST_S_RSRP|CA_POS_FIRST_S_RSRP_CNT|CA_POS_FIRST_S_RSRQ|CA_POS_FIRST_S_RSRQ_CNT|CA_SERVING_CNT|CALL_STATUS_2|CALL_STATUS_3|START_NON_BLOCK_CALL_CNT|RRC_SETUP_ATTEMPT|ERAB_SETUP_FAILURE|OVER_LOC_CALL_CNT|NONOVER_LOC_CALL_CNT|max.TILE_OVER_FLAG|OVER_DISTANCE_SUM|max.OVER_DISTANCE_MAX|RRC_SETUP_CNT|SETUP_TIME_SUM|SETUP_TIME_CNT|max.SETUP_TIME_MAX|MO_SETUP_TIME_SUM|MO_SETUP_TIME_CNT|max.MO_SETUP_TIME_MAX|MT_SETUP_TIME_SUM|MT_SETUP_TIME_CNT|max.MT_SETUP_TIME_MAX';
	...
	INSERT INTO gt_gw_main.sp_log VALUES(O_GT_DB,'SP_Sub_Generate_TAC_Start_LTE','rpt_tac_start', NOW());
	
	SET @rpt_target_table=CONCAT(GT_DB,'.rpt_tac_start_',STARTHOUR);
	SET @rpt_source_table=CONCAT(GT_DB,'.rpt_cell_start_',STARTHOUR);
	...
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	INSERT INTO gt_gw_main.sp_log VALUES(O_GT_DB,'SP_Sub_Generate_TAC_Start_LTE','rpt_tac_start_def', NOW());
	SET @rpt_target_table=CONCAT(GT_DB,'.rpt_tac_start_def_',STARTHOUR);
	SET @rpt_source_table=CONCAT(GT_DB,'.rpt_cell_start_',STARTHOUR);
	SET @SqlCmd=CONCAT('TRUNCATE TABLE ',@rpt_target_table,';');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	...
	INSERT INTO gt_gw_main.sp_log VALUES(O_GT_DB,'SP_Sub_Generate_TAC_Start_LTE','rpt_tac_start_dy', NOW());
	SET @rpt_target_table=CONCAT(GT_DB,'.rpt_tac_start_dy');
	SET @rpt_source_table=CONCAT(GT_DB,'.rpt_cell_start_',STARTHOUR);
	SET @SqlCmd=CONCAT('INSERT INTO ',@rpt_target_table,' 
					(
					',A_DY_KEY,
	...
	INSERT INTO gt_gw_main.sp_log VALUES(O_GT_DB,'SP_Sub_Generate_TAC_Start_LTE','rpt_tac_start_dy_def', NOW());
	SET @rpt_target_table=CONCAT(GT_DB,'.rpt_tac_start_dy_def');
	SET @rpt_source_table=CONCAT(GT_DB,'.rpt_cell_start_',STARTHOUR);
	SET @SqlCmd=CONCAT('INSERT INTO ',@rpt_target_table,' 
					(
					',A_DY_DEF_KEY,
	...
	INSERT INTO gt_gw_main.sp_log VALUES(O_GT_DB,'SP_Sub_Generate_TAC_Start_LTE',CONCAT('Done: ',TIMESTAMPDIFF(SECOND,START_TIME,SYSDATE()),' seconds.'), NOW());
END$$
DELIMITER ;
