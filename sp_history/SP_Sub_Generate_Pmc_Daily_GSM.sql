...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Sub_Generate_Pmc_Daily_GSM`(IN GT_DB VARCHAR(100))
BEGIN
	...
	INSERT INTO gt_gw_main.sp_log VALUES(O_GT_DB,'SP_Sub_Generate_Pmc_Daily_GSM','Start ', NOW());
	SET @SqlCmd=CONCAT('TRUNCATE TABLE ',DY_GT_DB,'.table_tile_pmc_gsm_dy_c ;');
	...
	SET @SqlCmd=CONCAT('INSERT INTO ',DY_GT_DB,'.`tmp_table_call_gsm_',WORKER_ID,'`  
									...
									GROUP BY DATA_DATE,POS_FIRST_BSC,POS_FIRST_CELL;');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	
	INSERT INTO gt_gw_main.sp_log VALUES(O_GT_DB,'SP_Sub_Generate_Pmc_Daily_GSM',CONCAT('Insert into table table_tile_pmc_gsm_dy_c'), NOW());
	
	SET @SqlCmd=CONCAT('INSERT INTO ',DY_GT_DB,'.`table_tile_pmc_gsm_dy_c`						
						SELECT 
							DATA_DATE,
							BSC_ID,
							CELL_ID,
						...
						(
							...
							FROM `',DY_GT_DB,'`.table_tile_fp_gsm_dy_c_def
							UNION ALL
							SELECT 
								DATA_DATE,
								BSC_ID,
								...
								NULL AS RE_ESTAB_ATT,
								NULL AS RE_ESTAB_FAIL
							FROM `',DY_GT_DB,'`.table_tile_start_gsm_dy_c_def
							UNION ALL
							SELECT 
								DATA_DATE,
								BSC_ID,
								...
								NULL AS RE_ESTAB_ATT,
								NULL AS RE_ESTAB_FAIL
							FROM `',DY_GT_DB,'`.table_tile_end_gsm_dy_c_def
							UNION ALL
							SELECT 
								DATA_DATE,
								BSC_ID,
						...
						GROUP BY DATA_DATE,BSC_ID,CELL_ID;');
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;	
	
	SET @SqlCmd=CONCAT('UPDATE ',DY_GT_DB,'.table_tile_pmc_gsm_dy_c a,',CURRENT_NT_DB,'.nt_cell_current_gsm B
	...
	INSERT INTO gt_gw_main.SP_LOG VALUES(O_GT_DB,'SP_Sub_Generate_Pmc_Daily_GSM',CONCAT('Done: ',TIMESTAMPDIFF(SECOND,START_TIME,SYSDATE()),' seconds.'), NOW());	
END$$
DELIMITER ;
