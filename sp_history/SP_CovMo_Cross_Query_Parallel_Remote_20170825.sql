...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_CovMo_Cross_Query_Parallel_Remote_20170825`(IN SQL_STR TEXT,IN POLYGON_STR LONGTEXT,IN IMSI_STR TEXT,IN CELL_GID SMALLINT(6),IN PU_ID INT(11),IN PLOYGON_ID VARCHAR(100),IN LIMT_RAW_COUNT SMALLINT(6))
BEGIN
	...
	IF CELL_GID>0 THEN 		
		...
		SET FSQL_STR=CONCAT(REPLACE(FSQL_STR,'@CELL_GID',CELL_STR),';');
			IF LOCATE('table_erab_volte_lte',FSQL_STR) > 0
			THEN
			    IF LOCATE('ERAB_START_SERVING_ENODEB',FSQL_STR) > 0
			        THEN SET FSQL_STR = REPLACE(FSQL_STR,'fact.enodeb_id','fact.ERAB_START_SERVING_ENODEB');
			             SET FSQL_STR = REPLACE(FSQL_STR,'fact.cell_id','fact.ERAB_START_SERVING_CELL');
				ELSEIF LOCATE('ERAB_END_SERVING_ENODEB',FSQL_STR) > 0
				THEN SET FSQL_STR = REPLACE(FSQL_STR,'fact.enodeb_id','ERAB_END_SERVING_ENODEB');
			             SET FSQL_STR = REPLACE(FSQL_STR,'fact.cell_id','fact.ERAB_END_SERVING_CELL');
			    END IF;
			ELSEIF LOCATE('table_call_failure_lte',FSQL_STR) > 0
			THEN
			    IF LOCATE('POS_LAST_S_ENODEB',FSQL_STR) > 0
			        THEN SET FSQL_STR = REPLACE(FSQL_STR,'fact.enodeb_id','fact.POS_LAST_S_ENODEB');
			             SET FSQL_STR = REPLACE(FSQL_STR,'fact.cell_id','fact.POS_LAST_S_CELL');
				ELSEIF LOCATE('POS_FIRST_S_ENODEB',FSQL_STR) > 0
				THEN SET FSQL_STR = REPLACE(FSQL_STR,'fact.enodeb_id','fact.POS_FIRST_S_ENODEB');
			             SET FSQL_STR = REPLACE(FSQL_STR,'fact.cell_id','fact.POS_FIRST_S_CELL');
				ELSEIF LOCATE('POS_FIRST_TILE',FSQL_STR) > 0
				THEN SET FSQL_STR = REPLACE(FSQL_STR,'fact.enodeb_id','fact.POS_FIRST_S_ENODEB');
			             SET FSQL_STR = REPLACE(FSQL_STR,'fact.cell_id','fact.POS_FIRST_S_CELL');
				ELSEIF LOCATE('POS_LAST_TILE',FSQL_STR) > 0
			        THEN SET FSQL_STR = REPLACE(FSQL_STR,'fact.enodeb_id','fact.POS_LAST_S_ENODEB');
			             SET FSQL_STR = REPLACE(FSQL_STR,'fact.cell_id','fact.POS_LAST_S_CELL');
			    END IF;
	
			ELSEIF LOCATE('table_call_lte',FSQL_STR) > 0
			THEN
			    IF LOCATE('POS_LAST_S_ENODEB',FSQL_STR) > 0
			        THEN SET FSQL_STR = REPLACE(FSQL_STR,'fact.enodeb_id','fact.POS_LAST_S_ENODEB');
			             SET FSQL_STR = REPLACE(FSQL_STR,'fact.cell_id','fact.POS_LAST_S_CELL');
	...
	SET @SqlCmd=IF(LIMT_RAW_COUNT>0 ,FSQL_STR,CONCAT(FSQL_STR,' LIMIT ',LIMT_RAW_COUNT));
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
