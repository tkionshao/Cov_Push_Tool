...
BEGIN
	...
	IF IFNULL(EXISTFLAG,0)=0 AND KIND != 'TMP' AND KIND != 'RERUN' AND KIND !='NT' AND KIND !='AP' AND KIND !='NT_DB_UMTS_LTE' THEN 
		...
		IF KIND = 'DAILY' THEN
			...
			-- CALL gt_gw_main.SP_CreateDB_PM(GT_DB,FILEDATE);
			CALL gt_gw_main.SP_CreateDB_Schema_Sub(GT_DB,'umts');
			CALL gt_gw_main.SP_Create_Merge_Rename_Table(GT_DB,'DY',EVENT_NUM,POS_NUM);
			-- CALL gt_gw_main.SP_CreateDB_PM_HUA(GT_DB);
-- 			CALL gt_gw_main.SP_CreateDB_PM_NSN(GT_DB);
			IF CUSTOMER_USER_FLAG = 'true' THEN
				CALL gt_gw_main.SP_Grant(GT_DB,2);
			END IF;
	...
	ELSEIF KIND = 'RERUN' THEN
		...
		CALL gt_gw_main.SP_CreateDB_Schema(CONCAT(GT_DB,'_rerun'),FILEDATE);
		-- CALL gt_gw_main.SP_CreateDB_PM(CONCAT(GT_DB,'_rerun'),FILEDATE);
		CALL gt_gw_main.SP_CreateDB_Schema_Sub(CONCAT(GT_DB,'_rerun'),'umts');
		CALL gt_gw_main.SP_Create_Merge_Rename_Table(CONCAT(GT_DB,'_rerun'),'DY',EVENT_NUM,POS_NUM);
		-- CALL gt_gw_main.SP_CreateDB_PM_HUA(CONCAT(GT_DB,'_rerun'));
-- 		CALL gt_gw_main.SP_CreateDB_PM_NSN(CONCAT(GT_DB,'_rerun'));
		CALL gt_gw_main.SP_CreateDB_lac(CONCAT(GT_DB,'_rerun'));
		IF DAILY_JUDGE=TRUE THEN
	...
	ELSEIF KIND = 'TMP' THEN
		...
		CALL gt_gw_main.SP_CreateDB_Schema(CONCAT(GT_DB,'_tmp'),FILEDATE);
		-- CALL gt_gw_main.SP_CreateDB_PM(CONCAT(GT_DB,'_tmp'),FILEDATE);
		CALL gt_gw_main.SP_CreateDB_Schema_Sub(CONCAT(GT_DB,'_tmp'),'umts');
		CALL gt_gw_main.SP_Create_Merge_Rename_Table(CONCAT(GT_DB,'_tmp'),'DY',EVENT_NUM,POS_NUM);
		-- CALL gt_gw_main.SP_CreateDB_PM_HUA(CONCAT(GT_DB,'_tmp'));
-- 		CALL gt_gw_main.SP_CreateDB_PM_NSN(CONCAT(GT_DB,'_tmp'));
		CALL gt_gw_main.SP_CreateDB_lac(CONCAT(GT_DB,'_tmp'));
		IF DAILY_JUDGE=TRUE THEN
