...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Generate_PU_LTE_Optus`(IN GT_DB VARCHAR(100),IN GT_COVMO VARCHAR(100))
BEGIN
	...
	SET @SqlCmd=CONCAT('INSERT INTO ',GT_DB,'.`pu_enodeb_mapping_',WORKER_ID,'` 
				...
				SELECT DISTINCT
				  ...
				  CLUSTER_NAME_REGION AS CLUSTER_NAME_REGION,
	...
	SET @SqlCmd=CONCAT('UPDATE ',GT_DB,'.`pu_enodeb_mapping_',WORKER_ID,'` A, ',GT_DB,'.cell_pu_mapping B
				SET
				A.PU_ID = B.PU_ID
	...
	EXECUTE Stmt;
	DEALLOCATE PREPARE Stmt;
	INSERT INTO gt_gw_main.sp_log VALUES(O_GT_DB,'SP_Generate_PU_LTE','Update PU_ID', NOW());
	SET @SqlCmd=CONCAT('UPDATE ',GT_DB,'.`nt_cell_current_lte` A, ',GT_DB,'.cell_pu_mapping B
				SET 
					A.PU_ID = B.PU_ID
				WHERE A.ENODEB_ID = B.ENODEB_ID;');
	PREPARE Stmt FROM @SqlCmd;
	...
	SET @SqlCmd=CONCAT('UPDATE ',GT_DB,'.`nt_cell_current_lte` A, ',GT_COVMO,'.`dim_sub_region_mapping` B
				SET 
	...
	SET @SqlCmd=CONCAT('	INSERT INTO ',GT_DB,'.`nt_pu_current`
				(PU_ID,PU_NAME,MCC,MNC,TECH_MASK,VEDNOR,SITE_CNT,CELL_CNT)
				SELECT  DISTINCT A.PU_ID,RNC_NAME AS PU_NAME,''505'',''02'',4 AS TECH_MASK ,ENODEB_VENDOR,
