...
CREATE DEFINER=`covmo`@`%` PROCEDURE `SP_Generate_IMSI_IMEI_Daily`(IN GT_DB_QRT VARCHAR(100),TECH_MASK TINYINT(2))
BEGIN
	DECLARE PU_ID INT;
	...
	SET @SqlCmd=CONCAT('CREATE TEMPORARY TABLE ',GT_DB,'.tmp_imsi_',PU_ID,'_',WORKER_ID,'
				SELECT IMSI,IMEI
				FROM ',GT_DB,'.table_call_imsi_',CASE TECH_MASK WHEN 1 THEN 'gsm' WHEN 2 THEN 'umts' WHEN 4 THEN 'lte' ELSE '' END,' 
				GROUP BY IMSI,IMEI;');
	SELECT @SqlCmd;
	PREPARE Stmt FROM @SqlCmd;
	EXECUTE Stmt;
